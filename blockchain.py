...
class Blockchain:
            ...
            with open('blockchain-{}.txt'.format(self.node_id), mode='r') as f:
                ...
                for block in blockchain:
                    converted_tx = [Transaction(
                        tx['sender'], tx['recipient'], tx['signature'], tx['amount'], tx['ehr']) for tx in block['transactions']]
                    updated_block = Block(
                ...
                for tx in open_transactions:
                    updated_transaction = Transaction(
                        tx['sender'], tx['recipient'], tx['signature'], tx['amount'], tx['ehr'])
    ...
    def add_transaction(self, recipient, sender, signature, ehr, amount=1.0, is_receiving=False):
        ...
        # if self.public_key == None:
        #     return False
        transaction = Transaction(sender, recipient, signature, amount, ehr)
                ...
                for node in self.__peer_nodes:
                    ...
                    try:
                        response = requests.post(url, json={
                                                 'sender': sender, 'recipient': recipient, 'amount': amount, 'signature': signature, 'ehr': ehr})
